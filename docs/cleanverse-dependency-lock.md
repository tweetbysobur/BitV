...
| `canTransfer` (CVA policy interface) | **UNCONFIRMED** | CVA transfer enforcement (not implemented, not attempted) | CVA guide confirms the argument list only; return type, visibility, mutability, and revert-vs-boolean rejection behavior are not confirmed | Ask Cleanverse for the exact function signature before ever declaring or calling it |
